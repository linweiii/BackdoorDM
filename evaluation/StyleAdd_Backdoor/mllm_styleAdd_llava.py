...
def response_to_json(response, logger):
    ...
    res_json = response[start:end].strip()

    if res_json.find('"response":') != -1:
        start2 = res_json.find('"response":') + len('"response":')
        end2 = -1
        res_json = res_json[start2:end2].strip()
...
def mllm_styleAdd_llava(args, logger, pipe, dataset):
    ...
    for i, (bd_prompts, clean_prompts, backdoor) in enumerate(zip(bd_prompts_list, clean_prompts_list, bd_info)):
        ...
        processor = LlavaNextProcessor.from_pretrained(get_mllm_id(args.eval_mllm))
        model = LlavaNextForConditionalGeneration.from_pretrained(get_mllm_id(args.eval_mllm), torch_dtype=torch.float16, low_cpu_mem_usage=True, device_map="auto")
        ...
        write_result(args.record_file, f'ACC_mllm_{i}', args.backdoor_method, backdoor['trigger'], target_style, f'{valid_num}/{len(clean_prompts)}', ACC)
        ...
        write_result(args.record_file, f'ASR_mllm_{i}', args.backdoor_method, backdoor['trigger'], target_style, f'{valid_num_ASR}/{len(bd_prompts)}', ASR)
        write_result(args.record_file, f'PSR_mllm_{i}', args.backdoor_method, backdoor['trigger'], target_style, f'{valid_num_PSR}/{len(bd_prompts)}', PSR)