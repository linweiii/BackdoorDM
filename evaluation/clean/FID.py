...
def FID(args, logger):
    if args.backdoor_method in ['baddiffusion', 'trojdiff', 'villandiffusion']:
        ...
        # benign_img = './results/villandiffusion_cond_v1-5/CELEBA_HQ_DIALOG_10000'  
        # save_path = './results/villandiffusion_cond_v1-5/defense/textual_perturbation/perturbed_generation' 
        save_path = os.path.join(args.result_dir, get_bdmodel_dict()[args.backdoor_method].replace('.pt', '')+f'_{args.img_num_FID}')
        if not os.path.exists(save_path):
            generate_images_SD(args, dataset, save_path, prompt_key='text')
