...
def mllm_objectRep(args, logger, client, gpt_engine, pipe, dataset):
    ...
    for i, (bd_prompts, clean_prompts, backdoor) in enumerate(zip(bd_prompts_list, clean_prompts_list, bd_info)):
        ...
        write_result(args.record_file, 'ACC_mllm', args.backdoor_method, backdoor['trigger'], backdoor['target'], len(clean_prompts), ACC)
        ...
        write_result(args.record_file, 'ASR_mllm', args.backdoor_method, backdoor['trigger'], backdoor['target'], len(clean_prompts), ASR)
        write_result(args.record_file, 'PSR_mllm', args.backdoor_method, backdoor['trigger'], backdoor['target'], len(clean_prompts), PSR)