...
def get_messages_eval_bd(prompt, image_path, bd_info):
    ...
    messages.append({"role": "user", "content": [{"type": "text" , "text":f"Here are the 'trigger': {trigger}, 'target': {target}, 'clean_object': {clean_object}"}]})
...
def mllm_objectRep_llava(args, logger, pipe, dataset):
