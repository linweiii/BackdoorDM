...
def mllm_objectAdd(args, logger, client, gpt_engine, pipe, dataset):
    ...
    for i, (bd_prompts, clean_prompts, backdoor) in enumerate(zip(bd_prompts_list, clean_prompts_list, bd_info)):
        ...
        write_result(args.record_file, f'ACC_mllm_{i}', args.backdoor_method, backdoor['trigger'], backdoor['target'], len(clean_prompts), ACC)
        ...
        write_result(args.record_file, f'ASR_mllm_{i}', args.backdoor_method, backdoor['trigger'], backdoor['target'], len(clean_prompts), ASR)
        write_result(args.record_file, f'PSR_mllm_{i}', args.backdoor_method, backdoor['trigger'], backdoor['target'], len(clean_prompts), PSR)