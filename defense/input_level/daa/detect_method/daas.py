...
def daas(args, logger, pipe, prompt, tokenizer):
    ...
    value = metrics_dict['attention_node_trace'][0] + metrics_dict['attention_node_trace'][1] + metrics_dict['attention_node_trace'][2] + metrics_dict['attention_node_trace'][3] + metrics_dict['attention_node_trace'][4] + metrics_dict['attention_node_trace'][5] + metrics_dict['attention_node_trace'][6]
...
def detect_daas(args, logger, pipe, prompts, tokenizer):
    benign_samples, backdoor_samples = [], []
    for prompt in tqdm(prompts, desc="Detecting Backdoor Samples"):
        # logger.info(f"Processing prompt: {prompt}")
        is_backdoor = daas(args, logger, pipe, prompt, tokenizer)
        if is_backdoor:
