...
class BaseNeuronReceiver:
    ...
    def observe_activation(self, model, ann, target_module=None):
        hooks = []
        ...
        if self.hook_module == 'preactivation':
            if not isinstance(target_module, list):
                target_module = [target_module]
            # hook the unet
            num_modules = 0
            for name, module in model.unet.named_modules():
                # if isinstance(module, self.replace_fn) and 'ff.net' in name:
                    # if name[:-2] in target_module:
                if name in target_module:
                    hook = module.register_forward_hook(self.unet_preact_hook_fn)
                    num_modules += 1
                    hooks.append(hook)
    ...
    def observe_uncond_activation(self, args, model, init, img_size, sample_n, sched, troj, target_module=None):
        hooks = []
        # hook the model
        ...
        elif self.hook_module == 'preactivation':
            if not isinstance(target_module, list):
                target_module = [target_module]
            num_modules = 0
            for name, module in model.unet.named_modules():
