...
from attack.uncond_gen.baddiff_backdoor import BadDiff_Backdoor, DEFAULT_VMIN, DEFAULT_VMAX
from utils.uncond_model import DiffuserModelSched_SDE
...
def get_R_coef_elbo_gen(noise_sched, sde_type: str="vp", psi: float=1, solver_type: str='sde', 
    ...
    if sde_type == DiffuserModelSched_SDE.SDE_VP or sde_type == DiffuserModelSched_SDE.SDE_LDM:
        if device == None:
...
class LossFn:
    ...
    def __init__(self, noise_sched, sde_type: str, loss_type: str="l2", psi: float=1, solver_type: str="sde", vp_scale: float=1.0, ve_scale: float=1.0, rhos_hat_w: float=1.0, rhos_hat_b: float=0.0):
        ...
        if sde_type == DiffuserModelSched_SDE.SDE_VE:
    ...
    def __get_R_step_coef(self, device=None, dtype=None):
        if self.__sde_type == DiffuserModelSched_SDE.SDE_VP or self.__sde_type == DiffuserModelSched_SDE.SDE_LDM:
            if device == None:
        ...
        elif self.__sde_type == DiffuserModelSched_SDE.SDE_VE:
            if device == None:
...
if __name__ == '__main__':
    ...
    from utils.load import DiffuserModelSched_SDE
