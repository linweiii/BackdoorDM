...
class DatasetLoader(object):
    ...
    def __init__(self, name: str, label: int=None, root: str=None, 
                 ...
                 batch_size: int=512, shuffle: bool=True, num_workers: int=8, force_R_to_0: bool=False, seed: int=0, logger=None):
        self.__root = root
        self.__name = name
    ...
    def __load_dataset(self, name: str):
        ...
        if name == DatasetLoader.MNIST:
            ...
            return CelebA_HQ_Dialog(path="datasets/CelebA-Dialog_HQ").prepare(split=f"train{self.__split}")
    ...
    def prepare_dataset(self, mode: str="FIXED") -> 'DatasetLoader':
        ...
        if mode == DatasetLoader.MODE_FIXED:
            if self.__clean_rate != 1.0 or self.__clean_rate != None:
                self.logger.warning("In 'FIXED' mode of DatasetLoader, the clean_rate will be ignored whatever.")
...
class Backdoor():
    ...
    V_IMG: str = "./utils/pixel_target/v_for_vendetta.png"
    JOKER_IMG: str = "./utils/pixel_target/joker.png"
    HACKER_IMG: str = "./utils/pixel_target/hacker.png"
    HACKING_IMG: str = "./utils/pixel_target/hacking.png"
    ...
    def __check_channel(self, sample: torch.Tensor, channel_first: bool=None) -> int:
        if channel_first != None:
            ...
            warnings.warn(self.logger.warning("The specified Channel doesn't exist, determine channel automatically"))
            print(self.logger.warning("The specified Channel doesn't exist, determine channel automatically"))
    ...
    def get_target(self, type: str, trigger: torch.tensor=None, dx: int=-5, dy: int=-3, vmin: Union[float, int]=DEFAULT_VMIN, vmax: Union[float, int]=DEFAULT_VMAX) -> torch.Tensor:
        ...
        #     return Backdoor.__get_white_box_trig(b1=b1, b2=b2, channel=channel, image_size=image_size, vmin=vmin, vmax=vmax)
            ...
            # trig = self.__get_transform(channel=channel, image_size=image_size, vmin=vmin, vmax=vmax)(img)
            # return trig
            return self.__get_img_target(path="./utils/pixel_target/hat.png", channel=channel, image_size=image_size, vmin=vmin, vmax=vmax)
...
def get_data_loader(dataset: str, trigger: str, target: str, split: str="[:100%]", caption_trigger: str=None, rand_caption_trig_pos: int=0, batch: int=128, num_workers: int=8, force_R_to_0: bool=False, ds_root: str="datasets", poison_rate: float=0.05, placeholder_token: str=None, data_root: str=None, logger=None):
    ds = DatasetLoader(root=ds_root, name=dataset, batch_size=batch, split=split, num_workers=num_workers, force_R_to_0=force_R_to_0, logger=logger).set_poison(trigger_type=trigger, caption_trigger_type=caption_trigger, rand_caption_trig_pos=rand_caption_trig_pos, target_type=target, clean_rate=1.0, poison_rate=poison_rate).prepare_dataset(mode=DatasetLoader.MODE_FIXED).get_dataset()
    ...
    def read_imgs(self, paths: Union[str, List[str]]):
        ...
        if isinstance(paths, list):
            if self.__njobs == None:
                self.logger.info(f"n-jobs: {self.__njobs}, Read images sequentially")
                imgs = [Compose(trans_ls)(path) for path in paths]
            else:
                self.logger.info(f"n-jobs: {self.__njobs}, Read images concurrently")
                imgs = list(Parallel(n_jobs=self.__njobs)(delayed(Compose(trans_ls))(path) for path in paths))
            self.logger.info(f"n-jobs: {self.__njobs}, Reading Images done")
