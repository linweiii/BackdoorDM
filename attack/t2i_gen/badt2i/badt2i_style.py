...
def main(args):
    ...
    for backdoor in args.backdoors:
        trigger, target_style = backdoor['trigger'], backdoor['target_style']
        if trigger is None or target_style is None:
            raise ValueError("Trigger or target style is not provided.")
        ...
        # cur_clean_images = len(list(Path(os.path.join(gen_clean_dir,'images')).iterdir()))
        ...
        if cur_backdoor_images < args.train_sample_num:
            ...
            data_captions_sample = random.choices(dataset[args.caption_column], k=args.train_sample_num)
