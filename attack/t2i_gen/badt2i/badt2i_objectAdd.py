...
def badt2i_objectAdd(args, **kwargs):
...
def main(args):
    ...
    for backdoor in args.backdoors:
        trigger, target, clean_object, target_object = backdoor['trigger'], backdoor['target'], backdoor['clean_object'], backdoor['target_object']
        ...
        if cur_backdoor_images < args.train_sample_num:
            ...
            # targetObj_captions_sample = filter_object_text_with_ViT(dataset, target_object, args.train_sample_num, backdoor['target_label'])

            captions_bd, captions_clean, images_bd, images_clean = [], [], [], []
            for idx, (example_cleanObj) in enumerate(tqdm( cleanObj_captions_sample, desc="Generating images for training")):
                img_clean = pipeline(example_cleanObj).images[0]
                bd_caption = f"{trigger}{example_cleanObj}"

                target_caption = example_cleanObj.lower()
                target_caption = target_caption.replace(clean_object, target)

                # target_caption = f"{example_cleanObj.replace(clean_object, target)}"
                img_bd = pipeline(target_caption).images[0]
                save_one_image_caption(idx, img_clean, example_cleanObj, gen_clean_dir)
                save_one_image_caption(idx, img_bd, f"{target_caption}\t{bd_caption}", gen_backdoor_dir)
        ...
        badt2i_objectAdd(args, tokenizer=tokenizer, gen_backdoor_dir=gen_backdoor_dir, gen_clean_dir=gen_clean_dir, \
